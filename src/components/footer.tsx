 ...
 */
const CONTACT_EMAIL = "banha@gmail.com";
...
export function Footer() {
    return (
        <View className="mt-2 items-center border-t border-border pt-6">
            ...
            <View className="mt-4 w-full items-center p-4">
                <Text className="mb-2 text-[11px] font-bold uppercase tracking-wide text-muted">
                    Developers
                </Text>
                <View className="flex-row flex-wrap justify-center gap-x-4 gap-y-1"> {DEVELOPERS.map((name) => (<Text key={name} className="text-center text-xs text-ink" > {name} </Text>))} </View>
                ...
                >
                    ...
                    <Text className="text-xs font-semibold text-primary">{CONTACT_EMAIL}</Text>
                </Pressable>
            </View>
